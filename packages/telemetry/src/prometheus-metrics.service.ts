...
export class PrometheusMetricsService implements OnModuleInit {
  ...
  // Business Metrics
  private jobApplicationsTotal: client.Counter<string>;
  private resumeGenerationTotal: client.Counter<string>;
  private aiServiceRateLimited: client.Counter<string>;
  private paymentTransactionsTotal: client.Counter<string>;
  private userChurnTotal: client.Counter<string>;
  private userActiveTotal: client.Gauge<string>;
  private apiQuotaExhausted: client.Counter<string>;
  private jobSearchDuration: client.Histogram<string>;

  // Database Metrics
  private dbConnectionPoolSize: client.Gauge<string>;
  private dbConnectionPoolUsed: client.Gauge<string>;
  private dbQueryDuration: client.Histogram<string>;
  private dbQueriesTotal: client.Counter<string>;

  // Queue Metrics (Bull)
  private bullQueueWaiting: client.Gauge<string>;
  private bullQueueActive: client.Gauge<string>;
  private bullQueueDelayed: client.Gauge<string>;
  private bullQueueCompleted: client.Counter<string>;
  private bullQueueFailed: client.Counter<string>;
  private bullQueueJobDuration: client.Histogram<string>;
  ...
  incrementJobApplications(status: 'success' | 'failed') {
  ...
  incrementResumeGeneration(status: 'success' | 'failed') {
  ...
  incrementPaymentTransaction(status: 'success' | 'failed') {
  ...
  setActiveUsers(count: number) {
    this.userActiveTotal.set(
      { service: process.env.SERVICE_NAME || 'unknown' },
      count,
    );
  }

  incrementAPIQuotaExhausted(userTier: string) {
  ...
  observeJobSearchDuration(durationSeconds: number) {
    this.jobSearchDuration.observe(
      { service: process.env.SERVICE_NAME || 'unknown' },
      durationSeconds,
    );
  }
  ...
  setDBConnectionPoolSize(database: string, size: number) {
    this.dbConnectionPoolSize.set(
      { database, service: process.env.SERVICE_NAME || 'unknown' },
      size,
    );
  }

  setDBConnectionPoolUsed(database: string, used: number) {
    this.dbConnectionPoolUsed.set(
      { database, service: process.env.SERVICE_NAME || 'unknown' },
      used,
    );
  }
  ...
  observeDBQueryDuration(
    ...
    this.dbQueryDuration.observe(
      { operation, table, service: process.env.SERVICE_NAME || 'unknown' },
      durationSeconds,
    );
  }

  incrementDBQueries(operation: string, table: string, status: string) {
  ...
  setBullQueueWaiting(queue: string, count: number) {
    this.bullQueueWaiting.set(
      { queue, service: process.env.SERVICE_NAME || 'unknown' },
      count,
    );
  }

  setBullQueueActive(queue: string, count: number) {
    this.bullQueueActive.set(
      { queue, service: process.env.SERVICE_NAME || 'unknown' },
      count,
    );
  }

  setBullQueueDelayed(queue: string, count: number) {
    this.bullQueueDelayed.set(
      { queue, service: process.env.SERVICE_NAME || 'unknown' },
      count,
    );
  }

  incrementBullQueueCompleted(queue: string) {
  ...
  incrementBullQueueFailed(queue: string) {
  ...
  observeBullQueueJobDuration(
    queue: string,
    jobName: string,
    durationSeconds: number,
  ) {
    this.bullQueueJobDuration.observe(
      { queue, job_name: jobName, service: process.env.SERVICE_NAME || 'unknown' },
      durationSeconds,
    );
  }
